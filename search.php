...
<div class="main_column column" id="main_column">
    ...
    if ($query == "") {
        ...
        if ($type == "username") {
            ...
            $names = explode(" ", $query);
             //if there two words, assume they are first and last names respectively
            if (count($names) == 3)
                $usersReturnQuery = mysqli_query($con, "SELECT * FROM users WHERE (first_name LIKE '$names[0]%' AND last_name LIKE '$names[2]%') AND user_closed='no'");// if query has one word only, search first names or last names
            else if (count($names) == 2)
                $usersReturnQuery = mysqli_query($con, "SELECT * FROM users WHERE (first_name LIKE '$names[0]%' AND last_name LIKE '$names[1]%') AND user_closed='no'");
            else
                $usersReturnQuery = mysqli_query($con, "SELECT * FROM users WHERE (first_name LIKE '$names[0]%' OR last_name LIKE '$names[0]%') AND user_closed='no'");
        }
        ...
        while($row = mysqli_fetch_array($usersReturnQuery)) {
            ...
            if($user['username'] != $row['username']) {
                //Generate button depending on frendship status
