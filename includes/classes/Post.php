...
class Post
{
    ...
    public function submitPost($body, $user_to)
    {
        ...
        if ($check_empty != "") {
            ...
            //If usern is not on own profile, user_to is 'none';
            if ($user_to == $added_by) {
                $user_to = 'none';
            }
            ...
            if ($user_to != 'none') {
                ...
                return $notification->insertNotification($returned_id, $user_to, "profile_post");
            }
                ...
                if ($user_logged_obj->isFriend($added_by)) {
                    ...
                    <script>
                        function toggle<?php echo $id; ?>() {
                            ...
                            if (!target.is('a')) { // если не кликнул на ссылку, то показать коментарии
                    ...
                    $interval = $start_date->diff($end_date); // Diffrence between to dates
                    if ($interval->y >= 1) {
                        if ($interval == 1) {
                        ...
                        if ($interval->i == 1) {
                            $time_message = $interval->h . " minute ago";
                        } else {
                            $time_message = $interval->h . " minutes ago";
                        }
                    } else {
                        if ($interval->s < 30) {
        ...
        if (mysqli_num_rows($data_query) > 0) {
            ...
            while ($row = mysqli_fetch_array($data_query)) {
                ...
                $str .= "<div class='status_post' onClick='javascript:toggle$id()'>
								...
								<div class='posted_by' style='color:#ACACAC'>
									<a href='$added_by'> $first_name $last_name </a> &nbsp&nbsp&nbsp;$time_message
									$delete_button
								</div>
								<div id='post_body'>$body<br><br><br>
								</div>
    ...
    public function getSinglePost($post_id)
    {
        ...
        if (mysqli_num_rows($data_query) > 0) {
            ...
            if ($user_logged_obj->isFriend($added_by)) {
                ...
                echo "<p>You cannot see this post becouse You are not friends with this user.</p>";
                return;
            }
        } else {
            echo "<p>No post found. If you ckicked a link, it may be broken.</p>";
        }
